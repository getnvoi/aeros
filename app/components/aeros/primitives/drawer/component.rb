module Aeros::Primitives::Drawer
  class Component < ::Aeros::ApplicationViewComponent
    ...
    style(:bg) do
      base { "bg-foreground/50 fixed inset-0 transition-opacity duration-300 opacity-0 pointer-events-none" }
    end

    style(:wrapper) do
      base { "fixed top-0 right-0 bottom-0 h-full transition-transform duration-300 translate-x-full bg-area-bg border-l border-area-border shadow-xl" }
    end
          ...
          <div data-action="<%= data_actions %>">
            ...
            >
              <div class="h-screen flex flex-col">
                <% if header %>
                  <div class="flex items-center justify-between p-4 border-b border-area-border flex-shrink-0">
                    <div class="flex-1 min-w-0"><%= header %></div>
                    <button data-action="click-><%= controller_name %>#close" type="button" class="ml-4 w-10 h-10 rounded-full bg-muted hover:bg-accent flex items-center justify-center flex-shrink-0 transition-colors">
                ...
                <% if footer %>
                  <div class="border-t border-area-border p-4 flex-shrink-0">
                    <%= footer %>
                  </div>
                <% end %>
              </div>
